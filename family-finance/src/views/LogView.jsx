...
import { fmt, fmtDate, fmtDayHeader } from '../lib/finance';
import { Chip, cardStyle } from '../components/ui';

export function LogView({ txs }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <p style={{ fontWeight: 900, fontSize: 18, color: '#1c1917', margin: 0 }}>Transaction Log</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input type="date" value={dateFilter} max={today}
            ...
            style={{ flex: 1, padding: '9px 12px', borderRadius: 10, border: '1.5px solid #e5e7eb', fontSize: 13, fontWeight: 600, color: '#1c1917', background: '#f9fafb', outline: 'none' }} />
          {dateFilter && (
      ...
      <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>{filtered.length} transaction{filtered.length !== 1 ? 's' : ''}</p>

      {filtered.length === 0 ? (
        <div style={{ ...cardStyle, textAlign: 'center' }}>
          <p style={{ fontSize: 28, margin: '8px 0 4px' }}>🔍</p>
          <p style={{ color: '#9ca3af', fontSize: 13, padding: '4px 0 16px' }}>No transactions match these filters.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {grouped.map(([date, dayTxs]) => {
            const { spent, income } = dayTotal(dayTxs);
            return (
              <div key={date}>
                {/* Day header */}
                    ...
                    <div key={tx.id} style={{ ...cardStyle, padding: '11px 14px', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1, overflow: 'hidden' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <p style={{ fontSize: 13, fontWeight: 800, color: '#1c1917', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 160 }}>{tx.category}</p>
                          <p style={{ fontSize: 14, fontWeight: 900, color: tx.type === 'Income' ? '#059669' : '#1c1917', margin: 0, flexShrink: 0 }}>
                        ...
                        <div style={{ display: 'flex', gap: 5 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, background: '#f3f4f6', color: '#6b7280', padding: '1px 7px', borderRadius: 10 }}>{tx.week}</span>
                          <span style={{ fontSize: 10, fontWeight: 700, background: '#f3f4f6', color: '#6b7280', padding: '1px 7px', borderRadius: 10 }}>{fmtDate(tx.date)}</span>
                          {tx.source === 'guest_portal' && (
                            <span style={{ fontSize: 10, fontWeight: 700, background: '#dbeafe', color: '#1d4ed8', padding: '1px 7px', borderRadius: 10 }}>
