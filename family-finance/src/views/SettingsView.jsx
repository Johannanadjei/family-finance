  ...
  return (
    <div style={{ ...cardStyle, borderRadius: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button onClick={() => setExpanded(e => !e)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: 18, padding: 4, lineHeight: 1 }}>
          {expanded ? '▲' : '▼'}
        </button>
      </div>
      ...
      {expanded && (
        <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid #f3f4f6' }}>
          ...
          {/* Portal link — only shown when enabled */}
          {guestSettings.enabled && (
            <div style={{ background: '#eff6ff', borderRadius: 14, padding: '14px', marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 11, color: '#3b82f6', margin: '0 0 10px', lineHeight: 1.5 }}>
                Share this link with your nanny or driver. It goes straight to the PIN screen — they never see the family dashboard.
              </p>
              <div style={{ background: '#fff', borderRadius: 10, padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8, border: '1.5px solid #bfdbfe' }}>
                ...
                <button onClick={handleCopy} style={{ flexShrink: 0, padding: '6px 12px', borderRadius: 8, border: 'none', background: copied ? '#059669' : '#2563eb', color: '#fff', fontWeight: 800, fontSize: 11, cursor: 'pointer' }}>
                  {copied ? '✓ Copied!' : 'Copy'}
                </button>
              </div>
            </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={guestSettings.label} onChange={e => set('label', e.target.value)} disabled={!guestSettings.enabled}
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={guestSettings.pin} onChange={e => set('pin', e.target.value.replace(/\D/g, '').slice(0, 4))}
          ...
          <div>
            <p style={{ fontSize: 11, fontWeight: 800, color: '#6b7280', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 1 }}>Allowed categories (empty = all)</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, opacity: guestSettings.enabled ? 1 : 0.4 }}>
              {FIXED_EXPENSES.map(e => {
                const selected = (guestSettings.allowedCategories || []).includes(e.category);
                return (
                  <button key={e.id} onClick={() => guestSettings.enabled && toggleCat(e.category)}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: 'linear-gradient(145deg,#064e3b,#0d7060)', borderRadius: 18, padding: '16px 18px', color: '#fff' }}>
        ...
        <p style={{ fontSize: 12, color: '#a7f3d0', margin: 0 }}>{HOUSEHOLD.adults} Adults · {HOUSEHOLD.children} Children · {HOUSEHOLD.month}</p>
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,.15)' }}>
          {[['Monthly Income', fmt(HOUSEHOLD.monthlyIncome)], ['Surplus Target', fmt(HOUSEHOLD.surplusTarget)]].map(([l, v]) => (
      ...
      {/* Preview guest portal — family only, goes back to dashboard */}
      {guestSettings.enabled && (
      ...
      <div style={cardStyle}>
        <p style={{ fontWeight: 800, fontSize: 14, color: '#1c1917', margin: '0 0 14px' }}>🔗 Spreadsheet Sync</p>
