...
import {
  ...
  calcSurplusLeft, calcTotalExpected, calcTotalReceived, calcAvailableNow,
...
export function useFinance({ centre, categories }) {
  ...
  const availableNow   = useMemo(() => calcAvailableNow(incomes, txs),                         [incomes, txs]);
  ...
  const surplusLeft    = useMemo(() => calcSurplusLeft(monthlyIncome, fixedTotal, variableSpent), [monthlyIncome, fixedTotal, variableSpent]);
