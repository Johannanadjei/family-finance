...
describe('useFinance — derived values', () => {
  ...
  it('spareMoney = monthlyIncome minus fixedTotal', async()=>{ const{result}=go(); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(44300); });
