...
describe('useFinance — derived values', () => {
  ...
  it('spareMoney = allIncome − max(fixedTotal, totalSpent)', async()=>{ const{result}=go(); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(29300); });
  it('spareMoney decreases by overspend when totalSpent exceeds fixedTotal', async()=>{ const extra=[...TXS,{id:'tx-3',type:'expense',amount:1000,category_name:'Other',date:'2026-05-20',week:'Week 3',currency:'GHS',source:'main_app',_optimistic:false}]; const{result}=go(extra); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(28800); });
