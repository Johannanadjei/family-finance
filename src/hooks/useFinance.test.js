...
describe('useFinance — derived values', () => {
  ...
  it('allIncome = totalReceived + totalIncome', async()=>{ const{result}=go(); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.allIncome).toBe(60000); });
  it('spareMoney = allIncome minus fixedTotal minus variableSpent', async()=>{ const{result}=go(); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(59300); });
  it('spareMoney decreases when variable expense added', async()=>{ const extra=[...TXS,{id:'tx-3',type:'expense',amount:1000,category_name:'Other',date:'2026-05-20',week:'Week 3',currency:'GHS',source:'main_app',_optimistic:false}]; const{result}=go(extra); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(58300); });
  it('spareMoney unchanged when known category expense added', async()=>{ const extra=[...TXS,{id:'tx-3',type:'expense',amount:1000,category_name:'Groceries',date:'2026-05-20',week:'Week 3',currency:'GHS',source:'main_app',_optimistic:false}]; const{result}=go(extra); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.spareMoney).toBe(59300); });
  it('availableNow = allIncome minus totalSpent', async()=>{ const{result}=go(); await waitFor(()=>expect(result.current.loading).toBe(false)); expect(result.current.availableNow).toBe(59800); });
