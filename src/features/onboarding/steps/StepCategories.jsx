/**
 * StepCategories.jsx
 * Onboarding Step 4 — Budget categories pre-filled from FIXED_EXPENSES defaults.
 * User can edit amounts, remove categories, or add custom ones.
 */
...
const DEFAULT_CATEGORIES = FIXED_EXPENSES.map(e => ({
  name:          e.category,
  icon:          e.icon,
  budget_amount: e.budget,
  is_fixed:      true,
}));
...
export function StepCategories({ data, onNext, onBack }) {
  ...
  const updateAmount = (i, val) =>
    setCats(prev => prev.map((c, idx) => idx === i ? { ...c, budget_amount: parseFloat(val) || 0 } : c));

  const removeCat = (i) => setCats(prev => prev.filter((_, idx) => idx !== i));

  const addCat = () => {
    if (!newName.trim() || !parseFloat(newAmount)) return;
    setCats(prev => [...prev, { name: newName.trim(), icon: '💸', budget_amount: parseFloat(newAmount), is_fixed: true }]);
    setNewName('');
    setNewAmount('');
  };
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 12px' }}>Review and adjust your monthly budget. Edit amounts or remove categories you do not need.</p>

      <div style={{ background: overBudget ? '#fef2f2' : '#f0fdf4', borderRadius: 12, padding: '10px 14px', marginBottom: 14 }}>
        <p style={{ fontSize: 13, fontWeight: 800, color: overBudget ? '#dc2626' : '#065f46', margin: 0 }}>
          Total budgeted: {fmt(total)} of {fmt(data.monthlyIncome)}
      ...
      <div style={{ maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
        {cats.map((c, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f9fafb', borderRadius: 10, padding: '8px 10px' }}>
            ...
            <input type="number" value={c.budget_amount} onChange={e => updateAmount(i, e.target.value)}
