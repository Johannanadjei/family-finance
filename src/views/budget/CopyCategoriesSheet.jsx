/**
 * views/budget/CopyCategoriesSheet.jsx — multi-select budget rollforward sheet (Phase 2C).
 *
 * The budget-categories analog of payday's CopyIncomeSheet. Lists the previous
 * month's categories, all checked by default — the primary "Yes, copy N" CTA on
 * the empty state is the all-in path, so the sheet exists for DE-selection. The
 * footer count tracks the selection live; with nothing selected the copy button
 * disables.
 *
 * Same chrome as CopyIncomeSheet — useModalChrome (scroll-lock + back-button
 * intercept) above the isOpen guard, portalled to <body>. Z-index 350.
 *
 * @param {boolean}   isOpen
 * @param {function}  onClose
 * @param {string}    lastMonthLabel — e.g. "May 2026"
 * @param {object[]}  categories     — previous-month budget categories
...
export function CopyCategoriesSheet({ isOpen, onClose, lastMonthLabel, categories = [], fmt, onCopy, copying = false }) {
  const [selected, setSelected] = useState(() => new Set());

  useEffect(() => {
    if (isOpen) setSelected(new Set(categories.map(c => c.id)));
  }, [isOpen, categories]);
    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {categories.map(cat => {
            ...
            return (
              <button
                key={cat.id}
                data-testid={`copy-category-${cat.id}`}
              ...
              >
                ...
                <span style={{ flex: 1, fontSize: 15, fontWeight: 800, color: 'var(--c-text, #1c1917)' }}>{cat.name}</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--c-muted, #6b7280)' }}>{fmt(cat.budget_amount)}</span>
              </button>
            );
          })}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 10 }}>
          <button
            data-testid="copy-categories-cancel-btn"
            onClick={onClose}
            disabled={copying}
            style={{
          ...
          <button
            data-testid="copy-categories-selected-btn"
