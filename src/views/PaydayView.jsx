    ...
    <div style={{ background: '#f5f3ff', borderRadius: 12, padding: '12px 14px' }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button onClick={handle}
          style={{ flex: 2, padding: '10px', borderRadius: 10, border: 'none', background: '#059669', color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
          Confirm Received
        </button>
        <button onClick={onCancel}
          style={{ flex: 1, padding: '10px', borderRadius: 10, border: '1.5px solid #e5e7eb', background: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer', color: '#6b7280' }}>
          Cancel
        </button>
      </div>
    </div>
...
/** Single income source card */
function IncomeCard({ income, onMarkReceived, onMarkPending }) {
    ...
    <div style={{ ...cardStyle, borderLeft: '4px solid ' + cfg.border }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 800, padding: '2px 9px', borderRadius: 20, background: cfg.bg, color: cfg.color }}>
              {cfg.label}
            </span>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        <div style={{ background: '#f9fafb', borderRadius: 12, padding: '10px 12px' }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 900, color: '#1c1917', margin: '2px 0 0' }}>{fmt(income.expectedAmount)}</p>
        </div>
        <div style={{ background: income.received ? '#f0fdf4' : '#f9fafb', borderRadius: 12, padding: '10px 12px' }}>
          <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>Received</p>
          <p style={{ fontSize: 16, fontWeight: 900, color: income.received ? '#059669' : '#9ca3af', margin: '2px 0 0' }}>
      ...
      {!income.received && income.expectedPayDay && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, background: '#f5f3ff', borderRadius: 10, padding: '8px 12px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {days !== null && (
              <p style={{ fontSize: 11, color: '#7c3aed', margin: '1px 0 0' }}>
      ...
      {income.received && income.actualPayDate && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, background: '#f0fdf4', borderRadius: 10, padding: '8px 12px' }}>
          ...
          <p style={{ fontSize: 12, fontWeight: 700, color: '#065f46', margin: 0 }}>
            Received on {fmtDate(income.actualPayDate)}
          </p>
        </div>
      )}
      ...
      {!income.received ? (
        confirming ? (
          <MarkReceivedForm
            income={income}
            onConfirm={(id, amt, date) => { onMarkReceived(id, amt, date); setConfirming(false); }}
            onCancel={() => setConfirming(false)}
          />
        ) : (
          <button onClick={() => setConfirming(true)}
            style={{ width: '100%', padding: '12px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer' }}>
            Mark as Received
          </button>
        )
      ) : (
        <button onClick={() => onMarkPending(income.id)}
          style={{ width: '100%', padding: '10px', borderRadius: 12, border: '1.5px solid #d1d5db', background: '#fff', fontSize: 12, fontWeight: 700, color: '#9ca3af', cursor: 'pointer' }}>
...
/** Main payday view */
export function PaydayView({ incomes, txs, totalExpected, totalReceived, availableNow, onMarkReceived, onMarkPending }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {/* Hero — available right now */}
      <div style={{ background: 'linear-gradient(145deg,#1e1b4b,#3730a3)', borderRadius: 20, padding: '20px', color: '#fff' }}>
        <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: 3, color: '#a5b4fc', textTransform: 'uppercase', margin: '0 0 4px' }}>
          Available Right Now
        </p>
        <p style={{ fontSize: 38, fontWeight: 900, margin: '0 0 4px', lineHeight: 1, color: availableNow < 0 ? '#fca5a5' : '#fff' }}>
          {fmt(Math.max(0, availableNow))}
        </p>
        <p style={{ fontSize: 12, color: '#a5b4fc', margin: '0 0 14px' }}>
          {fmt(totalReceived)} received · spend tracked this month
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 14 }}>
          {[
            ['Received', fmt(totalReceived), '#4ade80'],
            ['Pending',  fmt(totalPending),  '#fbbf24'],
            ['Expected', fmt(totalExpected), '#a5b4fc'],
          ].map(([l, v, c]) => (
