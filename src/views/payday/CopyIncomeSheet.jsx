/**
 * views/payday/CopyIncomeSheet.jsx — multi-select rollforward sheet (Phase 2B).
 *
 * Bottom sheet listing the previous month's income sources (one-off buckets
 * already excluded upstream). Every source is checked by default — the primary
 * "Yes, copy N" CTA on the empty state is the all-in path, so the sheet exists
 * for DE-selection. The footer count tracks the selection live; with nothing
 * selected the copy button disables.
 *
 * Same chrome as ConfirmSheet — useModalChrome (scroll-lock + back-button
 * intercept) above the isOpen guard, portalled to <body>. Z-index 350.
 *
 * @param {boolean}       isOpen
 * @param {function}      onClose
 * @param {string}        lastMonthLabel — e.g. "May 2026"
 * @param {object[]}      sources        — previous-month non-bucket income sources
 * @param {function}      fmt            — currency formatter
 * @param {function}      onCopy         — (selectedIds: string[]) => void
 * @param {boolean}       [copying]      — a copy is in flight
 */

import { useState, useEffect } from 'react';
import { createPortal }        from 'react-dom';
import { useModalChrome }      from '../../hooks/useModalChrome';

export function CopyIncomeSheet({ isOpen, onClose, lastMonthLabel, sources = [], fmt, onCopy, copying = false }) {
  // Selection of source ids. All checked when (re)opened.
  const [selected, setSelected] = useState(() => new Set());

  useEffect(() => {
    if (isOpen) setSelected(new Set(sources.map(s => s.id)));
  }, [isOpen, sources]);

  useModalChrome({ isOpen, onClose });

  if (!isOpen) return null;

  const toggle = (id) => setSelected(prev => {
    const next = new Set(prev);
    if (next.has(id)) next.delete(id); else next.add(id);
    return next;
  });

  const count    = selected.size;
  const disabled = copying || count === 0;

  return createPortal(
    <>
      <div onClick={onClose} aria-hidden="true" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', zIndex: 340 }} />

      <div
        role="dialog"
        aria-label={`Copy income from ${lastMonthLabel}`}
        data-testid="copy-income-sheet"
        data-modal-scrollable="true"
        style={{
          position: 'fixed', bottom: 0, left: 'max(0px, calc(50vw - 220px))',
          width: '100%', maxWidth: 440, background: 'var(--c-card, #fff)',
          borderRadius: '20px 20px 0 0', padding: '24px 20px calc(24px + env(safe-area-inset-bottom))',
          zIndex: 350, boxShadow: '0 -8px 32px rgba(0,0,0,.12)', maxHeight: '80vh', overflowY: 'auto',
        }}
      >
        <div style={{ width: 40, height: 4, background: 'var(--c-border, #e5e7eb)', borderRadius: 2, margin: '0 auto 20px' }} />

        <p style={{ fontSize: 18, fontWeight: 900, color: 'var(--c-text, #1c1917)', margin: '0 0 4px' }}>
          Copy from {lastMonthLabel}
        </p>
        <p style={{ fontSize: 13, color: 'var(--c-muted, #6b7280)', margin: '0 0 20px' }}>
          Pick the income sources to carry into this month.
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {sources.map(src => {
            const checked = selected.has(src.id);
            return (
              <button
                key={src.id}
                data-testid={`copy-source-${src.id}`}
                onClick={() => toggle(src.id)}
                aria-pressed={checked}
                style={{
                  display: 'flex', alignItems: 'center', gap: 12, width: '100%',
                  padding: '12px 14px', borderRadius: 12, cursor: 'pointer',
                  border: `1.5px solid ${checked ? 'var(--c-primary, #064e3b)' : 'var(--c-border, #e5e7eb)'}`,
                  background: checked ? 'var(--c-accent-light, #f0fdf4)' : 'var(--c-card, #fff)',
                  fontFamily: "'Nunito', sans-serif", textAlign: 'left',
                }}
              >
                <span aria-hidden="true" style={{
                  width: 20, height: 20, borderRadius: 6, flexShrink: 0,
                  border: `2px solid ${checked ? 'var(--c-primary, #064e3b)' : 'var(--c-border, #d1d5db)'}`,
                  background: checked ? 'var(--c-primary, #064e3b)' : 'transparent',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  {checked && (
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                  )}
                </span>
                <span style={{ fontSize: 18 }}>{src.icon}</span>
                <span style={{ flex: 1, fontSize: 15, fontWeight: 800, color: 'var(--c-text, #1c1917)' }}>{src.label}</span>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--c-muted, #6b7280)' }}>{fmt(src.expected_amount)}</span>
              </button>
            );
          })}
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 10 }}>
          <button
            data-testid="copy-cancel-btn"
            onClick={onClose}
            disabled={copying}
            style={{
              padding: '14px', borderRadius: 12, border: '1.5px solid var(--c-border, #e5e7eb)',
              background: 'var(--c-card, #ffffff)', fontSize: 14, fontWeight: 800,
              cursor: copying ? 'not-allowed' : 'pointer', color: 'var(--c-muted, #6b7280)',
              fontFamily: "'Nunito', sans-serif",
            }}
          >
            Cancel
          </button>
          <button
            data-testid="copy-selected-btn"
            onClick={() => onCopy(Array.from(selected))}
            disabled={disabled}
            style={{
              padding: '14px', borderRadius: 12, border: 'none',
              background: disabled ? 'var(--c-border, #e5e7eb)' : 'var(--c-primary, #064e3b)',
              color: disabled ? 'var(--c-muted, #9ca3af)' : 'var(--c-btn-text, #ffffff)',
              fontSize: 14, fontWeight: 800, cursor: disabled ? 'not-allowed' : 'pointer',
              fontFamily: "'Nunito', sans-serif",
            }}
          >
            {copying ? 'Copying…' : count === 0 ? 'Select at least one' : `Copy ${count} selected`}
          </button>
        </div>
      </div>
    </>,
    document.body
  );
}
