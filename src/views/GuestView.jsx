...
import { fmt, getCategoryIcon } from '../lib/finance';
...
/**
 * GuestView — expense logger for household staff (nanny, driver etc).
 * No family financial data is visible here.
 *
 * categories prop:
 *   - Passed from Dashboard (has real Supabase categories)
 *   - Empty array when accessed via guest portal URL (no auth)
 *   - Guest sees all categories when allowedCategories is empty
 */
export function GuestView({ guestUser, guestSettings, categories = [], onAddTransaction, onSignOut, isPortalUrl }) {
  ...
  // Resolve category names — use Supabase categories if available
  const allCatNames = categories.length > 0
    ? categories.map(c => c.name)
    : (guestSettings.allowedCategories?.length > 0 ? guestSettings.allowedCategories : []);

  const visibleCats = getGuestCategories(allCatNames, guestSettings.allowedCategories);

  const getIcon = (catName) => getCategoryIcon(catName, categories);
    ...
    <div style={{ fontFamily: "'Nunito', sans-serif", maxWidth: 440, margin: '0 auto', minHeight: '100vh', background: '#f0f9ff' }}>
      ...
      <div style={{ background: 'linear-gradient(145deg,#1e3a5f,#2563eb)', padding: '44px 20px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <p style={{ fontSize: 10, fontWeight: 800, letterSpacing: 3, color: '#93c5fd', textTransform: 'uppercase', margin: '0 0 4px' }}>Guest Portal</p>
          ...
          <button onClick={onSignOut}
            style={{ background: 'rgba(255,255,255,.12)', border: 'none', borderRadius: 10, padding: '7px 12px', color: '#bfdbfe', fontWeight: 800, fontSize: 11, cursor: 'pointer' }}>
            {isPortalUrl ? 'Sign out' : '← Back'}
          </button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 16 }}>
          {[["Today's entries", myTxs.filter(t => t.date === today).length], ['Total submitted', myTxs.length]].map(([l, v]) => (
      ...
      <div style={{ padding: '16px 16px 40px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: '18px', boxShadow: '0 1px 8px rgba(0,0,0,.06)' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {visibleCats.length === 0 && (
                <p style={{ fontSize: 13, color: '#9ca3af' }}>No categories available.</p>
              )}
              {visibleCats.map(cat => (
          ...
          <div style={{ marginBottom: 14 }}>
            <p style={{ fontSize: 11, fontWeight: 800, color: '#6b7280', margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: 1 }}>Amount</p>
          ...
          <button onClick={handleSubmit} disabled={!valid}
            ...
            {valid ? 'Submit Expense' : 'Fill in all fields'}
          </button>
        </div>

        {myTxs.length > 0 && (
          <div style={{ background: '#fff', borderRadius: 20, padding: '16px 18px', boxShadow: '0 1px 8px rgba(0,0,0,.06)' }}>
            <p style={{ fontWeight: 900, fontSize: 15, color: '#1c1917', margin: '0 0 14px' }}>Your submissions ({myTxs.length})</p>
